...
import "./library/UintCompressor.sol";
import "./library/OPKKeysCalcFast.sol";
import "./library/OPKdatasets.sol";

import "./OPKevents.sol";


contract OkamiPKsoon is OPKevents{
    using SafeMath for uint256;
    using NameFilter for string;
    using OPKKeysCalcFast for uint256;
    
	DiviesInterface constant private Divies = DiviesInterface(0xb47a8e5717D3346f63e2e9be40BE8D3Fe77b637F);
    PlayerBookInterface constant private PlayerBook = PlayerBookInterface(0x70BBe12C1Bae51011c6C6092Bfa1CfE948ce59e4);

    address private Community_Wallet1 = 0x52da4d1771d1ae96a3e9771D45f65A6cd6f265Fe;
    address private Community_Wallet2 = 0x0018Fa3a35a547CDaEF01C3156Ed0C41aCD2Dd0b;

//==============================================================================
//     _ _  _  |`. _     _ _ |_ | _  _  .
//    (_(_)| |~|~|(_||_|| (_||_)|(/__\  .  (game settings)
//=================_|===========================================================
    string constant public name = "Okami PK Soon(tm) Edition";
...
//==============================================================================
        ...
        public
    {
        ...
        // (soon, OPK) + (Pot , Referrals, Community)
...
//==============================================================================
    ...
    modifier onlyDevs(){
        require(
            //msg.sender == 0x00D8E8CCb4A29625D299798036825f3fa349f2b4 || //fortest
...
//=====================_|=======================================================
     ...
     */
    function icoPhaseCore(uint256 _pID, uint256 _eth, uint256 _team, uint256 _affID, OPKdatasets.EventReturns memory _eventData_)
        private
    {
        // setup local rID
        uint256 _rID = rID_;
        ...
        // send eth share to com, opk, affiliate, and fomo3d long
        ...
        // calculate pot share pot (eth = eth - (com share + pot swap share + aff share + opk share)) - gen
        uint256 _pot = (_eth.sub(((_eth.mul(14)) / 100).add((_eth.mul(fees_[_team].opk)) / 100))).sub(_gen);
     ...
     * is live.
     */
    function core(uint256 _pID, uint256 _eth, uint256 _affID, uint256 _team, OPKdatasets.EventReturns memory _eventData_)
        private
    {
        // setup local rID
        uint256 _rID = rID_;
...
//==============================================================================
     ...
     */
    function manageRoundAndPlayer(uint256 _pID, OPKdatasets.EventReturns memory _eventData_)
        private
        returns (OPKdatasets.EventReturns)
    {
        // setup local rID
     ...
     */
    function endTx(uint256 _rID, uint256 _pID, uint256 _team, uint256 _eth, uint256 _keys, OPKdatasets.EventReturns memory _eventData_)
        private
    {
